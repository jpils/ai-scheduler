...
impl TrainingWorkspace {
    ...
    pub fn verify_upet_models(
        project_dir: &Path,
        generation: u32,
        committee_members: usize,
    ) -> Result<(), String> {
        if committee_members == 0 {
            return Err("Cannot verify UPET models for zero committee members.".to_string());
        }
        ...
        for member_index in 0..committee_members {
            ...
            let model = models_dir.join(&member_name).join("model.pt");

            if !model.is_file() {
                return Err(format!(
                    "UPET training did not produce expected model: {}",
                    model.display()
                ));
            }
        }
